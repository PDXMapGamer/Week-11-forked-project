...
async function getExistingVote(userId, postId) {
  const { rows: existingVotes } = await db.query("SELECT * FROM votes WHERE user_id = $1 AND post_id = $2 LIMIT 1", [
    userId,
    postId,
  ]);
...
async function handleVote(userId, postId, newVote) {
  ...
  try {
    if (!userId) {
      errorMessage = "Cannot vote without being logged in";
    ...
    if (existingVote) {
      if (existingVote.vote === newVote) {
        ...
        await db.query("UPDATE votes SET vote = $1 WHERE id = $2", [newVote, existingVote.id]);
      }
    } else {
      // Insert a new vote
      await db.query("INSERT INTO votes (user_id, post_id, vote, vote_type) VALUES ($1, $2, $3, 'post')", [
        userId,
        postId,
        newVote,
      ]);
  ...
  return (
    <>
      <form className="flex items-center space-x-3 pl-3">
        <VoteButtons upvote={upvote} downvote={downvote} votes={votes} existingVote={existingVote} />
        {errorMessage ? <ToastComponent message={errorMessage} /> : <></>}
